...
Canonical time is ALWAYS tz-aware UTC datetime (every normalized ``ts``,
all six sources): one time type across the model, never mixed int/float
passthrough. Raw numerics/ISO strings convert; anything else rejects.
...
def normalize_alert(raw: Any) -> Alert:
    ...
    for extra in ("trace_id", "pod"):
        if data.get(extra) is not None:
            if not isinstance(data[extra], str):
                raise ValueError(
                    f"alert metadata {extra!r} must be a string "
                    f"(got {type(data[extra]).__name__})")
...
def normalize_metric(raw: Any) -> dict:
    ...
    ts = _coerce_ts_required(raw, "metric")
...
def normalize_deployment(raw: Any) -> dict:
    ...
    ts = _coerce_ts_required(raw, "deployment")
    rec = {
...
def normalize_k8s_event(raw: Any) -> dict:
    ...
    ts = _coerce_ts_required(raw, "k8s event")
